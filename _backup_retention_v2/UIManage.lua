...
function BuildPeoplePage()
    ...
    for _, profile in ipairs(NPC_PROFILES) do
        ...
        if isMet then
            ...
            -- 好感度计算（用于进度可视化）
            local affinityLevel = 0  -- 0-5
            local affinityLabel = "初识"
            local storyMax = 0
            local storyCur = 0
            -- 剧情NPC用剧情阶段算好感
            local storyProgress = (playerData_.npcStoryProgress or {})[profile.id]
            if storyProgress and storyProgress > 0 then
                storyCur = storyProgress
                storyMax = 5  -- 默认最大5阶段
                affinityLevel = math.min(5, storyCur)
            else
                -- 普通NPC用遭遇次数算好感
                affinityLevel = math.min(5, math.floor(eventCount / 1.5) + 1)
            end
            if affinityLevel <= 1 then affinityLabel = "初识"
            elseif affinityLevel == 2 then affinityLabel = "熟悉"
            elseif affinityLevel == 3 then affinityLabel = "信任"
            elseif affinityLevel == 4 then affinityLabel = "挚友"
            else affinityLabel = "羁绊" end
            ...
            table.insert(npcCards, UI.Panel {
                ...
                children = {
                    ...
                    UI.Panel {
                        ...
                        marginTop = 3, paddingLeft = 30,
                        children = {
                            UI.Label {
                                text = (affinityLevel >= 1 and "❤️" or "🤍")
                                    .. (affinityLevel >= 2 and "❤️" or "🤍")
                                    .. (affinityLevel >= 3 and "❤️" or "🤍")
                                    .. (affinityLevel >= 4 and "❤️" or "🤍")
                                    .. (affinityLevel >= 5 and "❤️" or "🤍"),
                                fontSize = 10,
                            },
                            UI.Label {
                                text = affinityLabel,
                                fontSize = 10, fontColor = affinityLevel >= 4 and { 255, 180, 200, 255 } or { 180, 160, 130, 200 },
                            },
                            storyMax > 0 and UI.Label {
                                text = "剧情 " .. storyCur .. "/" .. storyMax,
                                fontSize = 9, fontColor = { 170, 140, 255, 200 },
                                backgroundColor = { 140, 100, 220, 25 },
    ...
    elseif isActive then
        ...
        if cost then
            ...
            if hasDailyDeal then
                ...
                table.insert(bottomRow, UpgradePill("", "🏷️-" .. dailyDeal.pct .. "% 限时",
                    { 220, 80, 80, 60 }, { 255, 100, 100, 255 }))
