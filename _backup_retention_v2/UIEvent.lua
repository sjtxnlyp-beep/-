...
function BuildEventUI()
    ...
    if not evt.choices and not evt.candidate then
        table.insert(choiceBtns, UI.Button {
            text = "确认", width = "100%", height = 38, fontSize = 13, variant = "primary",
            onClick = function()
                local moneyBefore = playerData_.money
                if evt.effect then
...
function BuildMatchUI()
    ...
    if matchPhase_ == "intro" then
        ...
        -- P1-5: 出发前先进入战术选择阶段
        table.insert(children, UI.Button { text = "⚔️ 选择战术出发！", width = 200, height = 44, fontSize = 16, variant = "primary",
            onClick = function()
                PlaySFX("click")
    ...
    elseif matchPhase_ == "final_result" then
        -- 总战报（精简版：只显示每轮结果摘要，不显示叙事细节）
        local won = matchResult_ == "win"
        -- 最终结果置顶醒目显示（matchLog_最后一条即锦标赛/友谊赛最终结果）
        local finalEntry = matchLog_[#matchLog_]
        if finalEntry and finalEntry.text ~= "" then
            table.insert(children, UI.Label {
                text = finalEntry.text, fontSize = 16,
                fontColor = finalEntry.color or C.gold,
                width = "90%", textAlign = "center", whiteSpace = "normal",
            })
            table.insert(children, UI.Panel { height = 6 })
        end
        -- 各轮精简摘要（只保留标题行和胜负结果行）
        for i, log in ipairs(matchLog_) do
            if i < #matchLog_ then -- 跳过最后一条（已在顶部显示）
                local t = log.text
                if t:find("^──") or t:find("^🎉") or t:find("^💔") or t:find("^😤") or t:find("^🏅") then
                    table.insert(children, UI.Label { text = t, fontSize = 13, fontColor = log.color or C.text, width = "88%", whiteSpace = "normal" })
                end
            end
        end
        table.insert(children, UI.Panel { height = 6 })
