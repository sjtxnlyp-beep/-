...
local function BuildSeasonPassPanel()
    ...
    return UI.Panel {
        ...
        borderWidth = PX.border, borderColor = { 120, 100, 200, 80 },
        gap = 6,
        children = {
            UI.Panel {
    ...
    if rivalNpcs_ and #rivalNpcs_ > 0 and manageTab_ == "action" then
        ...
        table.insert(sbChildren, UI.Panel {
            ...
            children = {
                ...
                UI.Label {
                    text = topRival.name .. " 开业中 · 抢走约" .. (topRival.stealPct or 10) .. "%客流",
    ...
    local endDayBtn = UI.Panel {
        ...
        children = {
            UI.Panel {
                ...
                children = noAP and {
                    -- AP 耗尽：保留双行（说明性文字依然有用）
                    UI.Label { text = "✅ 结束今天", fontSize = 15, fontWeight = "bold",
                        fontColor = { 245, 255, 245, 255 } },
                    UI.Label { text = "行动点已用完，点击进入明天",
                        fontSize = 10, fontColor = { 205, 248, 215, 220 } },
                } or {
                    -- 还有AP：单行，天数内联
                    UI.Label { text = endMainText, fontSize = 15, fontWeight = "bold",
                        fontColor = { 245, 255, 245, 255 } },
                },
            },
        },
        ...
        children = {
            UI.Panel {
                ...
                children = noAP2 and {
                    ...
                    UI.Label { text = endMainText2, fontSize = 15, fontWeight = "bold",
                        fontColor = { 245, 255, 245, 255 } },
                },
            },
        },
...
function BuildUpgradeCard()
    local children = {}
    -- P2-1：当前过滤模式（all / available）
    local filter = playerData_.upgradeListFilter or "all"

    -- 过滤辅助：判断某升级项是否可见
    local function shouldShowUpgrade(key)
        ...
        if filter == "available" then
            return cur < maxLevels
        end
        -- "affordable"：未满级且买得起
        if filter == "affordable" then
            if cur >= maxLevels then return false end
            local cost = cfg.costs[cur + 1]
            return CanAffordCost(cost)
        end
        return true
    end

    -- 分组辅助：统计该分组内通过过滤的项目数（用于跳过空标题）
    local function countVisibleInGroup(keys)
        local n = 0
        for _, key in ipairs(keys) do
            if shouldShowUpgrade(key) then n = n + 1 end
        end
        return n
    end

    -- P2-1 过滤行（全部 / 可升级）
    local function FilterTabBtn(label, mode)
        local isActive = filter == mode
        return UI.Button {
            text = label,
            height = 24, paddingHorizontal = 10, fontSize = 11,
            borderRadius = PX.radius, borderWidth = PX.border,
            ...
            fontWeight = isActive and "bold" or "normal",
            onClick = function()
                playerData_.upgradeListFilter = mode; BuildUI()
            end,
        }
    end
    local filterRow = UI.Panel {
        ...
        paddingHorizontal = 10, paddingVertical = 4,
        children = {
            UI.Label { text = "筛选：", fontSize = 11, fontColor = C.textDim },
            FilterTabBtn("全部", "all"),
            FilterTabBtn("可升级", "available"),
            FilterTabBtn("买得起", "affordable"),
        },
    }

    -- 局部升级组构建（支持过滤）
    local function BuildFilteredGroup(keys, dest)
        for _, key in ipairs(keys) do
            if shouldShowUpgrade(key) then
                local card = BuildUpgradeItemCard(key)
                if card then table.insert(dest, card) end
            end
        end
    end
    ...
    -- 标题行 + 过滤器（分两行避免 PanelHeader width=100% 挤掉过滤器）
    table.insert(children, PanelHeader("升级·集市", { icon = nil, compact = true }))
    table.insert(children, filterRow)
    BuildFilteredGroup(UPGRADE_ORDER, children)

    -- ── 社区投资分组（过滤后无内容则跳过标题）──
    if countVisibleInGroup(UPGRADE_COMMUNITY) > 0 then
        table.insert(children, UI.Divider { spacing = 6 })
        table.insert(children, PanelHeader("社区投资", { icon = "🏘️", compact = true, color = C.gold }))
        BuildFilteredGroup(UPGRADE_COMMUNITY, children)
    end

    -- ── 文化空间分组（过滤后无内容则跳过标题）──
    if countVisibleInGroup(UPGRADE_CULTURE) > 0 then
        table.insert(children, UI.Divider { spacing = 6 })
        table.insert(children, PanelHeader("文化空间", { icon = "🎭", compact = true, color = { 220, 140, 80, 255 } }))
        BuildFilteredGroup(UPGRADE_CULTURE, children)
    end
