...
function PrestigeSystem.DoPrestige(targetCityId)
    ...
    -- 叙事系统：转生告别仪式（在数据重置前执行，留存NPC告别对话）
    local PersonalStory = PersonalStory  -- 引用全局
    if PersonalStory and PersonalStory.PRESTIGE_FAREWELL then
        local farewellOk, farewellErr = pcall(function()
            local fromCity = prestigeRecord.fromCity or "wakandaville"
            local toCity = nextCity and nextCity.id or "unknown"
            local farewellData = PersonalStory.PRESTIGE_FAREWELL.getDialogues(fromCity, toCity)
            local npcFarewells = PersonalStory.PRESTIGE_FAREWELL.getNpcFarewells()
            -- 存入全局变量，供 UI 层在转生过渡动画中展示
            pendingFarewellCeremony_ = {
                dialogues = farewellData,
                npcFarewells = npcFarewells,
                fromCity = fromCity,
                toCity = toCity,
                scene = "farewell",
            }
        ...
        if not farewellOk then
            log:Write(LOG_ERROR, "[DoPrestige] farewell ceremony error: " .. tostring(farewellErr))
        end
    end
