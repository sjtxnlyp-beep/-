...
local CityTeamData = require("CityTeamData")

--- 初始化候选队员池（根据当前城市加载对应角色）
--- @param cityId string|nil 城市ID，nil 则使用 playerData_.currentCity 或默认 wakandaville
function InitCandidatePool(cityId)
    local currentCity = cityId or (playerData_ and playerData_.currentCity) or "wakandaville"

    -- 起始城市使用原始固定角色
    if currentCity == "wakandaville" then
        CANDIDATE_POOL = {
        ...
        -- 如果是转生回来，额外加入新面孔增加趣味
        if playerData_ and (playerData_.prestigeCount or 0) > 0 then
            local extras = CityTeamData.wakandaville_extra or {}
            for _, c in ipairs(extras) do
                table.insert(CANDIDATE_POOL, c)
            end
        end
    else
        -- 非起始城市：从 CityTeamData 加载该城市的特色队员
        local cityPool = CityTeamData.GetCandidatesForCity(currentCity)
        if cityPool and #cityPool > 0 then
            -- 深拷贝避免修改原始数据
            CANDIDATE_POOL = {}
            for _, c in ipairs(cityPool) do
                local copy = {}
                for k, v in pairs(c) do copy[k] = v end
                copy.mood = 100  -- 新队员初始心情满
                table.insert(CANDIDATE_POOL, copy)
            end
        else
            -- Fallback：如果城市数据缺失，用起始城市的额外角色
            CANDIDATE_POOL = {}
            local extras = CityTeamData.wakandaville_extra or {}
            for _, c in ipairs(extras) do
                local copy = {}
...
function ResetGame()
    ...
    playerData_ = {
        ...
        -- 角色组合事件系统
        comboTriggered = {},
        -- 非洲文化图鉴系统（loreUnlocked 不重置，跨存档保留收集进度）
        loreNewCount = 0,
        loreLastUnlock = nil,
        loreMaxItemTier = 0,
        loreSpecialItems = {},
        -- 旅行者NPC系统
        currentTraveler = nil,
        travelerLastVisitDay = 0,
        travelerHistory = {},
        travelerBuffs = nil,
        travelerStoriesHeard = 0,
        travelerItemsBought = 0,
        travelerRecruitBonus = 0,
        -- 装饰槽位系统（Batch 3）
        decoSlots = {},          -- { [slotIdx] = itemUID }
        decoSlotsMax = 3,        -- 初始3个装饰位
        -- 网吧装修自定义
        cafeCustom = {
            wallpaper = "default",   -- default/lagos/nairobi/accra/dakar/capetown
            lighting  = "warm",      -- warm/cool/neon
            floor     = "cement",    -- cement/wood
        },
        -- 训练小游戏个人记录板
        trainRecords = {
            aim    = { score = 0, combo = 0 },
            ...
            memory = { correct = 0, rounds = 0 },
            comm   = { correct = 0, rounds = 0, speed = 0 },
        },
        trainPlayCount = 0,  -- 总训练/重玩次数
        -- 图鉴系统
        collection = {},        -- { [itemId] = dayUnlocked }
        collectionTiers = {},   -- { [tierId] = dayCompleted }
        -- 高潮日系统
        climaxState = {
            activeToday = nil,
            actionsUsed = {},
    ...
    InitCandidatePool()
    -- 感情线系统初始化
    if RomanceSystem and RomanceSystem.InitBonds then
        RomanceSystem.InitBonds(playerData_)
    end
    teamMembers_ = {}; eventLog_ = {}; currentChapter_ = 1; npcJournal_ = {}
    pendingChapterMilestones_ = nil; pendingChapterAdvance_ = nil
    recruitReplaceIdx_ = nil; dismissConfirmIdx_ = nil; trainNoAPIdx_ = nil
    replayTrainMode_ = false  -- 训练场重玩模式标志
    ...
    challengeDifficulty_ = 0.5; challengeNPCScore_ = 0; challengeMultiplier_ = 1.5; challengeRoundResult_ = nil
...
function HandleUpdate(eventType, eventData)
    ...
    -- 章节漫画 splash 面板自动推进
    if currentPhase_ == PHASE_CHAPTER_COMIC and chapterComicSplashTimer_ > 0 then
        -- 先处理文字延迟显示
        if chapterComicSplashDelay_ > 0 then
            chapterComicSplashDelay_ = chapterComicSplashDelay_ - dt
            if chapterComicSplashDelay_ <= 0 then
                chapterComicSplashDelay_ = 0
                pcall(BuildUI)  -- 刷新UI显示延迟文字
            end
        end
        -- 自动推进倒计时
        chapterComicSplashTimer_ = chapterComicSplashTimer_ - dt
        if chapterComicSplashTimer_ <= 0 then
    ...
    -- 策略卡5秒未选择自动折叠
    if currentPhase_ == PHASE_MANAGE and not transition_.active
       and playerData_ and playerData_.todayStrategy
       and not playerData_.strategyChosen
       and not stratCardCollapsed_
       and stratCardShowTime_ and stratCardShowTime_ > 0
       and (gameTime_ - stratCardShowTime_) >= 5.0 then
        stratCardCollapsed_ = true
        pcall(BuildUI)
    end
    ...
    if currentPhase_ == PHASE_DIALOGUE and not typewriter_.done then
        ...
        CinematicDialogue.UpdateFadeIn(dt)
        UpdateTypewriter(dt)
        -- 同步 CinematicDialogue 状态到旧 typewriter_ 变量
        typewriter_.done = CinematicDialogue.IsDone()
        -- 独白模式：文本由渐显动画控制，不需要逐帧更新label
        -- 对话模式：逐字打字机更新 dialogueText
        local isMono = CinematicDialogue.IsMonologue()
        if not isMono then
        ...
        -- 独白打字机完成后更新提示文字
        if isMono and CinematicDialogue.IsDone() then
            local hintLabel = uiRoot_ and uiRoot_:FindById("monoHint")
            if hintLabel then
                local hintText = (dialogueIndex_ < #currentDialogues_) and "点击继续" or "点击完成"
    ...
    if currentPhase_ == PHASE_TRAIN and trainActive_ and trainPhase_ == "playing" and trainMode_ == "react" then
        ...
        elseif reactPhaseState_ == "show" then
            ...
            -- 闪现模式：0.4秒后方向消失，用增量更新代替全量 BuildUI
            if reactFlash_ and math.abs(reactFlashTimer_ - 0.4) < dt * 1.1 then
                local dirIcon = uiRoot_ and uiRoot_:FindById("reactDirIcon")
                local dirHint = uiRoot_ and uiRoot_:FindById("reactDirHint")
                if dirIcon and dirHint then
                    dirIcon:SetText("?")
                    dirHint:SetText("记住方向！")
                    dirHint:SetFontColor(C.accent)
                else
                    BuildUI()  -- fallback
                end
            end
    ...
    if currentPhase_ == PHASE_TRAIN and trainActive_ and trainPhase_ == "playing" and trainMode_ == "memory" then
        ...
        if routePhaseState_ == "show" then
            routeShowTimer_ = routeShowTimer_ + dt
            local interval = 0.6
            if routeShowTimer_ >= interval then
                routeShowTimer_ = 0
                routeShowIdx_ = routeShowIdx_ + 1
                if routeShowIdx_ > #routePath_ then
                    -- 展示完毕，进入输入阶段
                    routeShowIdx_ = #routePath_
                    routePhaseState_ = "input"
                    routePlayerPath_ = {}
                end
                BuildUI()
            end
    ...
    -- 训练更新（指挥通讯模式）
    if currentPhase_ == PHASE_TRAIN and trainActive_ and trainPhase_ == "playing" and trainMode_ == "comm" then
        trainTimer_ = trainTimer_ + dt
        if commPhaseState_ == "wait" then
            commTimer_ = commTimer_ + dt
            if commTimer_ >= 1.0 then
        ...
        elseif commPhaseState_ == "scroll" then
            commTimer_ = commTimer_ + dt
            -- 给玩家一段时间找出目标（根据速度递减时间）
            local timeLimit = math.max(2.5, 5.0 / commSpeed_)
            if commTimer_ >= timeLimit and not commAnswered_ then
                -- 超时 — 情报流失
                commMissed_ = true
