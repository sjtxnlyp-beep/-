...
function ResetGame()
    ...
    recruitReplaceIdx_ = nil; dismissConfirmIdx_ = nil
...
function HandleUpdate(eventType, eventData)
    ...
    -- 恢复网吧事件选择后的滚动位置（延迟一帧，等待布局计算完成）
    if restoreManageScroll_ and uiRoot_ then
        local sv = uiRoot_:FindById("manage-scroll")
        if sv then
            sv:SetScroll(restoreManageScroll_.x, restoreManageScroll_.y)
            restoreManageScroll_ = nil
        end
    end
