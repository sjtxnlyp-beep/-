...
function PrestigeSystem.CalcPrestigeGain()
    ...
    local dayPart = math.floor(day / 5) * 3  -- 每5天贡献3点
...
function PrestigeSystem.CanPrestige()
    ...
    -- 最低条件：经营满15天 且 总收入 >= $2,000
    if day < 15 then
        return false, "需要经营满15天（当前第" .. day .. "天）", 0
    end
    if totalEarnings < 2000 then
        return false, "需要累计收入 ≥ $2,000（当前 $" .. totalEarnings .. "）", 0
    end
...
function PrestigeSystem.DoPrestige(targetCityId)
    ...
    playerData_.currentCity = nextCity and nextCity.id or "wakandaville"
    ...
    local cityName = nextCity and nextCity.name or "新城市"
    AddLog("🌟 【转生】恭喜！获得 " .. gain .. " 商会名誉！")
    AddLog("🏙️ 前往「" .. cityName .. "」开设新网吧！")
    ...
    return true, "成功转生至「" .. cityName .. "」，获得 " .. gain .. " 商会名誉！"
end
