...
function BuildDaySummaryPopup()
    ...
    -- 页面标题
    local pageTitles = { "📊 收支摘要", "📖 今日故事", "🔔 状态总结" }
    local pageIdx = page
    if not hasStory and page >= 2 then pageIdx = 3 end  -- 跳过故事页时映射标题
    local titleText = (pageTitles[pageIdx] or pageTitles[1])
